...
\subsection{Model Comparison: XGBoost vs PyTorch MLP}
...
\begin{table}[H]
\centering
\caption{Per-Class Delay Prediction: XGBoost vs PyTorch MLP (v1 Dataset, 5-fold out-of-fold)}
...
\begin{table}[H]
\centering
\caption{SLA Violation Detection: XGBoost vs PyTorch MLP (Tuned Thresholds)}
...
\begin{itemize}
    ...
    \item The PyTorch MLP matches XGBoost per class (delay $R^2$ within 0.002 on every class) but does not beat it, while trading a little precision for higher recall on the harder Silver and Bronze classes. XGBoost is retained as the recommended model for its equal accuracy, interpretability, and lower training cost.
\end{itemize}
