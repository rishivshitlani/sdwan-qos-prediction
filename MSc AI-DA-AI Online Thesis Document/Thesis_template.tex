...
For the Zenodo dataset, XGBoost achieved the strongest throughput result among the tested models, with a cross-validated $R^2$ of approximately 0.756. The dataset is useful for link capacity prediction, but it does not contain QoS class variation. For the BNN-UPC dataset, XGBoost acts as the strongest explainable tree-based baseline, while the PyTorch feedforward neural network is the main neural-network path. Both models achieved similar performance on the log-transformed average delay target, with cross-validated $R^2$ values around 0.766--0.769.

The thesis also addresses a key evaluation issue: a single global metric can hide poor performance for one QoS class. Therefore, the BNN-UPC model is evaluated by QoS class, scheduling scenario, and scheduling policy. The results show that Gold and Silver delay are predicted much more accurately than Bronze delay. Finally, a Layer 3 allocation recommender is implemented. It tests candidate WFQ bandwidth profiles and recommends the least-violating Gold/Silver/Bronze allocation under SLA thresholds. The current recommender results are based on the trained delay model already evaluated in this thesis. The next planned enhancement is to compare allocation decisions from both XGBoost and the PyTorch MLP. This connects QoS prediction to the original SD-WAN decision question: how much bandwidth should be allocated to each QoS class?
	\end{abstracts}
...
This dataset is used for the main QoS scheduling and allocation experiments.
...
The completed experiments show four main findings:

\begin{itemize}
    \item Zenodo is useful for link capacity prediction, but not for per-class QoS allocation.
    \item BNN-UPC provides a useful per-class QoS scheduling dataset.
    \item Global BNN-UPC metrics hide class-level differences; Bronze is much harder to predict than Gold and Silver.
    \item The Layer 3 recommender connects prediction to the original SD-WAN allocation question, and the next step is to compare XGBoost-based and MLP-based allocation decisions.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Multi-Metric Prediction:} Extend the BNN-UPC work from delay prediction to jitter, packet loss, and throughput.
    ...
    \item \textbf{Better Bronze Prediction:} Improve recall for Bronze SLA violations, which are harder because Bronze delay is heavy-tailed.
